...
// El refresh token se guarda como SHA-256 (nunca en claro). No se usa bcrypt: solo lee los
// primeros 72 bytes y todos los JWT de un usuario comparten ese inicio.
...
export async function refreshTokens(token: string) {
  ...
  const user = await findUserWithRefreshToken(payload.sub);
